...
(async () => {
  ...
  const agendamentoCodigo = 'AGD000128';
  const cdNome = 'CENTRAL';
  const consultaUrl = 'https://brisanetlog.com.br/consultar-status.html';
  const resposta = 'aceito';
  const novaData = '2025-09-29';
  const novoHorario = '15:00';
  const comentario = 'Aceito a nova data.';

  // Email de RESPOSTA DE REAGENDAMENTO
  const html = `
