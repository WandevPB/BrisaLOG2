...
(async () => {
  ...
  const agendamentoCodigo = 'AGD000123';
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, #FF6B35 0%, #FF8C42 50%, #FF9F66 100%); color: white; border-radius: 10px; padding: 30px 20px; text-align: center;">
        <h1 style="margin: 0; font-size: 26px;">Solicitação Recebida com Sucesso!</h1>
        <p style="font-size: 18px; margin: 10px 0 0 0;">Seu agendamento foi registrado no BrisaLOG Portal</p>
      </div>
      <div style="background: #fff; border-radius: 10px; margin-top: 20px; padding: 30px 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
        ...
        <p>Seu agendamento foi finalizado com sucesso no Centro de Distribuição <b>${cdNome}</b>.</p>
        ...
        <p>Guarde este código! Ele será necessário para consultar o status da sua solicitação a qualquer momento.</p>
        ...
        <div style="background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 5px; padding: 15px; margin: 20px 0; color: #856404;">
          <b>Importante:</b> Não responda este e-mail. Em caso de dúvidas, entre em contato com o CD responsável.
        </div>
      </div>
  ...
  try {
    const result = await emailService.transporter.sendMail({
      ...
      to: fornecedorEmail,
      subject: '[BrisaLOG] Solicitação Finalizada com Sucesso',
      html
    });
