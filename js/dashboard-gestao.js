class DashboardGestao {
    ...
    async transferirCD(agendamentoId) {
        try {
                ...
                <div id="modal-transferir-cd" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div class="px-6 pb-6 flex gap-3">
                            ...
                            <button onclick="dashboardGestao.confirmarTransferencia(${agendamentoId})" 
            ...
            alert('Erro ao carregar dados para transferência');
        }
    }
    ...
    async confirmarTransferencia(agendamentoId) {
        try {
            ...
            // Validações
            if (!novoCdId) {
                alert('Selecione o novo CD de destino');
                return;
            }

            if (!motivo || motivo.length < 10) {
                alert('Digite um motivo detalhado (mínimo 10 caracteres)');
                return;
            }
            ...
            const token = sessionStorage.getItem('token');
            const response = await fetch(`${API_BASE_URL}/api/agendamentos/${agendamento.codigo}/transferir-cd`, {
                method: 'POST',
                headers: {
            ...
            alert(`✅ Agendamento transferido de "${result.agendamento.cdAnterior}" para "${result.agendamento.cdNovo}" com sucesso!\n\n📧 Email enviado ao transportador.`);
