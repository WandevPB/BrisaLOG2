...
class CDDashboard {
        ...
        detailContent.innerHTML = `
            <div class="space-y-4">
                ...
                <div class="space-y-3 mt-6">
                    <!-- Ações de Fluxo Normal -->
                    <div class="flex flex-wrap gap-3 justify-end">
                        ${this.getActionButtons(agendamento)}
                    </div>
                    
                    <!-- Opções Administrativas -->
                    ${agendamento.status !== 'entregue' && agendamento.status !== 'cancelado' ? `
                        <div class="border-t border-gray-200 pt-4">
                            <p class="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-2">
                                <i class="fas fa-user-shield mr-1"></i>Opções Administrativas
                            </p>
                            <div class="flex flex-wrap gap-2">
                                <button onclick="dashboard.transferirCD(${agendamento.id})" 
                                    class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-all font-medium text-sm shadow-md">
                                    <i class="fas fa-exchange-alt mr-2"></i>Transferir Local de Entrega
                                </button>
                            </div>
                        </div>
                    ` : ''}
    ...
    async transferirCD(agendamentoId) {
        try {
            ...
            const token = sessionStorage.getItem('token');
            const responseCds = await fetch(`${getApiBaseUrl()}/api/cds`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            const cds = await responseCds.json();
                ...
                <div id="modal-transferir-cd" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
                        <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6 rounded-t-2xl">
                            ...
                            <p class="text-blue-100 mt-2 text-sm">Agendamento: ${agendamento.codigo}</p>
                        </div>
                        ...
                        <div class="p-6 space-y-4">
                            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                                <div class="flex items-start">
                                    ...
                                    <div>
                                        <p class="text-yellow-800 font-medium text-sm">CD Atual: <strong>${agendamento.cdNome || 'N/A'}</strong></p>
                            ...
                            <div>
                                ...
                                <select id="select-novo-cd" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                                    ...
                                    ${cds.map(cd => cd.id !== agendamento.cdId ? `<option value="${cd.id}">${cd.nome}</option>` : '').join('')}
                                </select>
                            </div>
                        ...
                        <div class="px-6 pb-6 flex gap-3">
                            ...
                            <button onclick="dashboard.confirmarTransferencia(${agendamentoId})" 
    ...
    async confirmarTransferencia(agendamentoId) {
        try {
            ...
            // Buscar agendamento para pegar o código
            const agendamento = this.agendamentos.find(a => a.id === agendamentoId);
            if (!agendamento) {
            ...
            const token = sessionStorage.getItem('token');
            const response = await fetch(`${getApiBaseUrl()}/api/agendamentos/${agendamento.codigo}/transferir-cd`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
