...
class AgendamentoForm {
    ...
    async buscarTransportadorPorCNPJ() {
        ...
        const cnpjFormatado = cnpjInput.value.trim(); // Pega o valor COM formatação
        const cnpjLimpo = cnpjFormatado.replace(/[^\d]/g, ''); // Remove formatação apenas para validar
        
        // Só buscar se tiver pelo menos 14 dígitos
        if (cnpjLimpo.length < 14) {
            console.log('⚠️ CNPJ incompleto:', cnpjLimpo, 'length:', cnpjLimpo.length);
            return;
        }

        try {
            console.log('🔍 Buscando transportador com CNPJ formatado:', cnpjFormatado);
            
            // Enviar CNPJ COM formatação (como está no banco)
            const url = `${getApiBaseUrl()}/api/transportador/buscar-por-cnpj/${encodeURIComponent(cnpjFormatado)}`;
